...
export const API_PATH = {
  ...
  UPDATE_NEWS_VIEWS: "/article/view",
};
...
export async function updateNewsViews(id: string) {
  const url = API_PATH.UPDATE_NEWS_VIEWS;
...
export function useAPIGetTopics(
  querys: ITopicQuery = { current: 1, pageSize: 100, categoryName: "BGA" }
) {
  const url = API_PATH.GET_TOPICS;
  return useSWR<IPageData<ITopic>>(
    [url, querys],
    () =>
      http.get(API_PATH.GET_TOPICS, {
        params: querys,
      }),
    SWR_OPTIONS
  );
