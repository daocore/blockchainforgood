...
import { ICreateVote, IEmail, IVote, IVoteResult, IVoteResultParams } from "../types";
...
export function useAPIVoteDetail(id: string) {
  ...
  const fetcher = async () => {
    const response = (await  Q(http.get(url, {
      params: querys,
    }))) as IVote
    let loc = {
      city: "-",
    ...
    const location =
    response.event.online === 1 ? "ONLINE" : loc?.city + ", " + loc?.area;
...
export function APIGetVoteResult(params: IVoteResultParams) {
  return Q<IVoteResult[]>(http.get(API_PATH.GET_VOTE_RESULT, {
    params
  }))
}

export function useAPIGetVoteResult(params: IVoteResultParams) {
  return useSWR<IVoteResult[]>([API_PATH.GET_VOTE_RESULT, params], () => APIGetVoteResult(params), { refreshInterval: 10000 })
}